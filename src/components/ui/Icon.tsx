...
/**
 * Named imports only — combined with `optimizePackageImports` in next.config,
 * this keeps a single icon from pulling the whole set into the bundle.
 *
 * lucide-react v1 removed all brand marks, so GitHub and LinkedIn are inlined
 * below rather than pinned to an older major.
 */
...
function GithubMark({ size, className }: { size: number; className?: string }) {
  return (
    <svg {...brandProps(size)} className={className}>
...
function LinkedinMark({ size, className }: { size: number; className?: string }) {
  return (
    <svg {...brandProps(size)} className={className}>
...
export function Icon({
  ...
  if (name === "Linkedin")
    return <LinkedinMark size={size} className={className} />;
